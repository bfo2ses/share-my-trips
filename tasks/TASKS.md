...
| COR-010 | Session expirée (401) : le handler `onError` ne vide que le token module — sessionStorage, state AuthProvider et cache restent en état connecté (UI figée, pas de redirection /login, token mort réarmé au refresh). Fix suggéré : callback `onUnauthorized` passé à `makeClient(token)` et câblé sur `logout()`, en supprimant le token module partagé | `[ ]` à corriger |
